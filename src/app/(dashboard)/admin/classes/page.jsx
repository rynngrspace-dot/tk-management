...
export default function AdminClassesPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      const url = isEdit ? `/api/classes/${selectedClass.id}` : "/api/classes"
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-white/80 backdrop-blur-sm p-5 rounded-2xl shadow-sm border border-white/60">
        ...
        <Button 
          ...
          className="mt-4 sm:mt-0 gradient-primary text-white rounded-xl shadow-md shadow-indigo-500/20 hover:shadow-lg hover:opacity-95 transition-all duration-200"
        >
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="rounded-2xl sm:max-w-md">
          ...
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              ...
              <p className="text-[10px] text-slate-400">
                {isEdit ? "ID Kelas (Primary Key) tidak dapat diubah setelah dibuat." : "ID Kelas bersifat unik dan digunakan sebagai kode kelompok."}
              </p>
            </div>
            <div className="space-y-2">
          ...
          <DialogFooter className="gap-2 sm:gap-0">
            <Button variant="outline" onClick={() => setIsDialogOpen(false)} className="rounded-xl h-11">Batal</Button>
            <Button 
              className="gradient-primary text-white rounded-xl h-11 min-w-[100px]" 
              onClick={handleSave}
