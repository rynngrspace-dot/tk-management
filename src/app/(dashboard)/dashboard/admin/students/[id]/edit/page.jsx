...
import { ArrowLeft, Loader2, FileText, ShieldCheck, User, Phone, HeartPulse, Mail, KeyRound } from "lucide-react"
import { toast } from "sonner"
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select"

export default function AdminEditStudentPage({ params }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setIsLoading(true)
        const [resStudent, resClasses] = await Promise.all([
          fetch(`/api/students`),
          fetch("/api/classes")
        ])
        
        if (!resStudent.ok || !resClasses.ok) throw new Error("Gagal mengambil data")
        
        const students = await resStudent.json()
        const classesData = await resClasses.json()
        setClasses(classesData)
        
        const myStudent = students.find(s => s.id === parseInt(id))
        if (!myStudent) {
        ...
        const linkedParent = myStudent.parents?.[0]
        setFormData({
          name: myStudent.name || "",
          nis: myStudent.nis || "",
          classId: myStudent.classId || "",
          gender: myStudent.gender || "",
          dateOfBirth: myStudent.dateOfBirth ? new Date(myStudent.dateOfBirth).toISOString().split('T')[0] : "",
          parentName: myStudent.parentName || (linkedParent ? linkedParent.name : ""),
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      ...
      <div className="flex items-center gap-4 bg-white/80 backdrop-blur-sm p-5 rounded-2xl shadow-sm border border-white/60">
        ...
        <div>
          <h2 className="text-xl font-bold tracking-tight text-slate-800">Edit Data Siswa</h2>
          <p className="text-xs text-slate-500 mt-0.5">Ubah informasi profil akademik dan data diri siswa</p>
        </div>
      </div>
          ...
          <TabsContent value="keluarga" className="space-y-6 mt-6">
            ...
            <div className="bg-indigo-50/30 p-5 rounded-2xl border border-indigo-100/60 space-y-4">
              ...
              {formData.createParentAccount ? (
                // Parent account exists or is selected to be created
                <div className="space-y-4">
                  <div className="space-y-2">
                    <Label className="text-xs font-medium text-slate-500 uppercase tracking-wider flex items-center gap-1.5">
                      <Mail className="w-3.5 h-3.5 text-slate-400" /> Email Login Orang Tua <span className="text-rose-500">*</span>
                    </Label>
                    <Input 
                      type="email"
                  ...
                  <div className="space-y-2">
                    <Label className="text-xs font-medium text-slate-500 uppercase tracking-wider flex items-center gap-1.5">
                      <KeyRound className="w-3.5 h-3.5 text-slate-400" /> Password Akun <span className="text-slate-400 normal-case font-normal">(Isi jika ingin mengubah password akun orang tua)</span>
                    </Label>
                    <Input 
                      type="text"
                ...
                <div className="space-y-4">
                  <p className="text-[11px] text-indigo-600/80 leading-normal font-medium">
                    Siswa ini belum memiliki akun login orang tua yang terhubung. Centang di bawah untuk membuat akun baru secara otomatis.
                  </p>
